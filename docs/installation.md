...
=== "Standalone binary"
    ...
    !!! warning
        ...
        If Blockasaurus runs under a **restricted capability bounding set** (for example a
        hardened `systemd` unit, or a container that drops capabilities), use
        `setcap 'cap_net_bind_service=+p' ./blockasaurus` instead. Blockasaurus raises the
...
Blockasaurus docker images are deployed to GitHub Container Registry (`ghcr.io/chrissnell/blockasaurus`).
